...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 35
        versionCode = 59
        versionName = "0.10.0" // 0.10.0·0.9.x 缓存/预读线收官（合并 0.9.1~0.9.5）：① 章节缓存治理——ChapterCacheManager 加 7 天 TTL + 300MB LRU 容量上限，过期自动刷新、不再无限膨胀（0.9.1）；② 双向多章预读——读章后后台预取「下3+上1」章，并发≤2、已缓存跳过，翻章几乎零等待（0.9.2）；③ 翻章无缝——命中缓存瞬时换章、滚动触底自动续读下一章（下一章已预读，无感切换）（0.9.3）；④ 进书即热——详情页后台预热前 2 章，点开阅读第一眼即零等待（0.9.4）；⑤ 整本离线下载——书籍详情页「下载整本」按钮，并发拉取全部章节落本地、带进度，断网可读；设置页新增预读深度（下/上）与触底续读开关（0.9.5）。整条链路不改主站、不写主站，遵守物理隔离
