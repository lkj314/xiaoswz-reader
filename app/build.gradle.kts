...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 35
        versionCode = 98
        versionName = "0.19.3" // 0.19.2·书圈用户需知（首次进入书圈强制弹窗，勾选【我知道了】方可进入，DataStore 持久化只弹一次）：书圈专区升级为个人钱包（净值/持仓/理财收益）；新增理财产品市场（董事发行多书币资产包，用户认购份额、收益每日结算打入钱包）；书圈稳定币（1万枚硬顶、真实B000001 1:10背书、守恒、概率衰减 max(0.0005,0.001·(1−n/10000)²)；董事每日抓取/调拨）；退市阈值14天连负或累计−30%。
